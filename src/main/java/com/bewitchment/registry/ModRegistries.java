...
public class ModRegistries {
		...
		try {
			for (Field f : ModObjects.class.getFields()) {
				...
				if (obj instanceof Block) {
					...
					for (Block block : (Block[]) obj) event.getRegistry().register(block);
			}
	...
	public static void registerEntities(RegistryEvent.Register<EntityEntry> event) {
		...
		for (int i = 0; i < 4; i++)
			LootTableList.register(new ResourceLocation(Bewitchment.MODID, "entities/demon" + i));
		...
		LootTableList.register(new ResourceLocation(Bewitchment.MODID, "entities/werewolf"));
		LootTableList.register(new ResourceLocation(Bewitchment.MODID, "entities/bafometyr"));
		LootTableList.register(new ResourceLocation(Bewitchment.MODID, "entities/cleaver"));
	}
		...
		event.getRegistry().register(new Tarot(new ResourceLocation(Bewitchment.MODID, "illusioner"), new ResourceLocation(Bewitchment.MODID, "textures/gui/tarot/05illusioner.png")) {
			...
			public int getNumber(EntityPlayer player) {
				...
				for (PotionEffect effect : player.getActivePotionEffects())
					if (effect.getAmplifier() > max) max = effect.getAmplifier();
				return max;
			}
		});
